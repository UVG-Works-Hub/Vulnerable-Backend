const getNombreMedicos = "SELECT medicos.nombre , COUNT(*) as num_pacientes FROM medicos JOIN pacientes ON medicos.medico_id = pacientes.paciente_id GROUP BY medicos.nombre ORDER BY num_pacientes DESC LIMIT 10";
...
const getTopHospitales = "SELECT lugares.nombre, COUNT(*) as total_pacientes FROM pacientes INNER JOIN visitas ON pacientes.paciente_id = visitas.paciente_id INNER JOIN lugares ON visitas.lugar_id = lugares.lugar_id GROUP BY lugares.nombre ORDER BY total_pacientes DESC LIMIT 10;";
